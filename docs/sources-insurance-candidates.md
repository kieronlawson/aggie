# Sources — insurance vertical feed candidates (phase 4 research)

Research output for the intel-aggregator insurance-vertical source expansion. **Seeded 2026-07-29** (phase 4): NatLawReview Insurance, JD Supra Insurance (via the working `docsRSSfeed.aspx` endpoint — the `feed.aspx` 503 below turned out to be the endpoint, not egress), Insurance Journal, plus crawl targets InsuranceNewsNet, NAIC newsroom, and NY DFS. See `INSURANCE_SOURCES` in `src/registry/seed.ts` and the phase 4 entry in `SEED_NOTES` for what was skipped and why.
